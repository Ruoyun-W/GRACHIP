...
def evaluate(args, model, tokenizer, prefix="", evaluate=True):
   ...
   # eval_task_names = "dna"
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                outputs = model(**inputs)[0][:,0,:].cpu().numpy()
                if(out is None):
                    out=outputs
                else:
                    out=np.append(out,outputs,axis=0)
        with open(args.predict_dir, 'wb') as f:
            np.save(f, out)
